...
class Node {
public:
  Node(int id, int parent, std::vector<Particle> particles, double x0,
       double y0, double width, double height, int x_coord, int y_coord,
       int depth)
      : _id(id), _parent(parent), _x0(x0), _y0(y0), _width(width),
        _height(height), _particles(particles), _is_leaf(true),
        _incoming_coeff(), _outgoing_coeff(), _x_coord(x_coord),
        _y_coord(y_coord), _depth(depth) {}
  ...
  const std::vector<int> &get_close_neighbours() const { return close_nodes; }
  ...
  void del_particles() { _particles.clear(); }

  int const size() const { return _particles.size(); }

  std::complex<double> const get_center() const {
    return std::complex<double>(_x0 + _width / 2, _y0 + _height / 2);
  }

  bool const is_leaf() const { return _is_leaf; }
  ...
  unsigned int const &get_x_coord() const { return _x_coord; }

  unsigned int const &get_y_coord() const { return _y_coord; }

  int const &get_depth() const { return _depth; }
  ...
  void butput() {
    ...
    for (auto i : _outgoing_coeff) {
      std::cout << i << ' ';
    }
    std::cout << std::endl << "incoming coeff " << std::endl;
    for (auto i : _incoming_coeff) {
      std::cout << i << ' ';
    }
...
public:
  ...
  QuadTree(std::vector<Particle> particles) {
    double _x0 =
        (*std::min_element(particles.begin(), particles.end(),
                           [&](Particle a, Particle b) {
                             return a.get_pos().real() < b.get_pos().real();
                           }))
            .get_pos()
            .real();
    double _y0 =
        (*std::min_element(particles.begin(), particles.end(),
                           [&](Particle a, Particle b) {
                             return a.get_pos().imag() < b.get_pos().imag();
                           }))
            .get_pos()
            .imag();
    double _width =
    ...
    node_map.emplace_back(
        Node(0, 0, particles, _x0, _y0, _width, _height, 0, 0, 0));
  }
  ...
  Node &get_node(int id) { return node_map[id]; }
