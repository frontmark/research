  ...
  while (id <= cluster_nodes.size()) {
    ...
    if (cl_node.is_dead() == false) {
      if (cl_node.get_neighours().size() == 1) {
        cl_node.die();
        cluster_positions.push_back(
            ClusterPositioner(0, id, cl_node.get_neighours()[0],
                              cl_node.get_neighbors_distances()[0]));
        ClusterNode neigh = cluster_nodes[cl_node.get_neighours()[0]];
        cl_node.die();
        neigh.add_neighbour(cl_node.get_representant(),
                            cl_node.get_neighbors_distances()[0]);
      }
    }
    id++;
...
void ClusterGraph::cluster_bridges() {
  ...
  while (id <= cluster_nodes.size()) {
    ...
    if (cl_node.is_dead() == false) {
      ...
      if (neighbors.size() == 2) {
        cl_node.die();
        cluster_positions.push_back(ClusterPositioner(
            1, id, cl_node.get_neighours()[0], cl_node.get_neighours()[1],
            cl_node.get_neighbors_distances()[0],
            cl_node.get_neighbors_distances()[1]));
        ClusterNode &neigh1 = cluster_nodes[cl_node.get_neighours()[0]];
        ClusterNode &neigh2 = cluster_nodes[cl_node.get_neighours()[1]];
        neigh1.add_neighbour(neigh2.get_representant(),
                             cl_node.get_neighbors_distances()[0] +
                                 cl_node.get_neighbors_distances()[1]);
        neigh2.add_neighbour(neigh1.get_representant(),
                             cl_node.get_neighbors_distances()[0] +
                                 cl_node.get_neighbors_distances()[1]);
    ...
    if (central_node.is_dead() == false) {
      ...
      if (neighbors.size() >= min_neigh) {
        ...
        for (auto neigh_id : neighbors) {
          ...
          cluster_positions.push_back(ClusterPositioner(
              2, neigh_id, id,
              central_node.get_neighbors_distances()[neigh_number]));
          central_node.add_to_cluster(
              neigh, central_node.get_neighbors_distances()[neigh_number]);
          neigh_number++;
        }
      }
    }
...
Graph ClusterGraph::get_graph() {
  ...
  for (int n = 0; n < cluster_nodes.size(); n++) {
    if (cluster_nodes[n].is_dead() == false) {
      g.add_particle(Particle(cluster_nodes[n].get_pos(),
                              cluster_nodes[n].get_charge(), id),
                     ParticleData());
  ...
  for (auto node : cluster_nodes) {
    if (node.is_dead() == false) {
      for (int m = 0; m < node.get_neighours().size(); m++) {
        if (node.get_representant() < node.get_neighours()[m] &
            cluster_nodes[node.get_neighours()[m]].is_dead() == false) {
          Edge edge;
...
std::vector<Compl>
ClusterGraph::get_unclustered_layout(std::vector<Compl> clustered_layout) {
  ...
  for (std::vector<ClusterPositioner>::reverse_iterator i =
           cluster_positions.rbegin();
       i != cluster_positions.rend(); ++i) {
    pos_map[i->get_id()] =
        i->get_position(pos_map, cluster_nodes[i->get_id()].get_neighours());
  }
