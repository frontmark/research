...
struct hash_pair {
  size_t operator()(std::pair<Compl, double> a) {
    return std::hash<double>{}(a.first.imag()) ^
           std::hash<double>{}(a.first.real()) ^ std::hash<double>{}(a.second);
  }
};
...
public:
  ...
  void calc_ffm_forces(int terms = 3, int thresh = 10, int const node_id = 0,
                       double strength = .5);
  ...
  void calc_gravity_forces(double gravity) {
    ...
    for (Particle p : _particles) {
      ...
      if (dist > 0.0001) {
        _forces[p.get_id()] -=
            gravity * p.get_pos() * p.get_charge() / std::sqrt(dist);
      }
    }
  };
  ...
  void calc_edge_forces(double edge_factor, const std::vector<Edge> &edges,
                        std::vector<int> degree) {
    for (auto edge : edges) {
      double distance = std::max(std::abs(_particles[edge.source].get_pos() -
                                          _particles[edge.target].get_pos()),
                                 0.0001);
      double opt_edge_length = 1;
      Compl force = double(degree[edge.source] + 1) *
                    double(degree[edge.target] + 1) * Compl(edge_factor, 0) *
                    (_particles[edge.source].get_pos() -
                     _particles[edge.target].get_pos()) *
  ...
  void calc_edge_forces_2(double edge_factor, const std::vector<Edge> &edges) {
    for (auto edge : edges) {
      double distance = std::max(std::abs(_particles[edge.source].get_pos() -
                                 ...
                                 0.0001);
      double opt_edge_length = 1;
      Compl force = Compl(edge_factor, 0) *
                    (_particles[edge.source].get_pos() -
                     _particles[edge.target].get_pos()) *
                    (1.0 - distance) / distance;
      _forces[edge.source] += force;
