...
{
    public:
        ClusterNode(int representant, std::vector<int> neighbors, std::vector<double> neighbors_distances, std::complex< double> repr_pos, double charge){
        ...
        void add_neighbour(int neighbour_id, int distance){
            if (neighbour_id!= _representant){
                if (std::find(_neighbors.begin(), _neighbors.end(), neighbour_id) == _neighbors.end()) {
            ...
            for (int n=0; n<other.get_neighours().size();n++){
                ...
                if (id!= _representant){
                    if (std::find(_neighbors.begin(), _neighbors.end(), id) == _neighbors.end()) {
                        ...
                        _neighbors_distances.push_back(other_distance+other.get_neighbors_distances()[id]);
                    }
                }
            }
        }
...
class ClusterPositioner
{
    public:
        ...
        ClusterPositioner(int type, int id, int neigh1, double dist1){_type=type; _id=id; _neigh1 = neigh1; _distance1 = dist1;}
        ClusterPositioner(int type, int id, int neigh1, int neigh2, double dist1, double dist2){_type=type; _id=id; _neigh1 = neigh1; _neigh2 = neigh2;_distance1 = dist1; _distance2=dist2;}
        int get_id(){return _id;}
        Compl get_position(std::unordered_map<int, Compl> & pos_map, std::vector<int> neighbors = {}){
            if (_type==0){
...
public:
    ClusterGraph(Graph graph){
        ...
        for (auto particle: graph.get_particles()){
            ...
            cluster_nodes.push_back(ClusterNode(particle.get_id(), neighbors[i], distances, particle.get_pos(), particle.get_charge()));
            i++;
        }
    };
    ...
    Graph get_graph();
    std::vector<Compl> get_unclustered_layout(std::vector<Compl> clustered_layout);
