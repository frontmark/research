...
class Particle {
public:
  Particle(std::complex<double> pos, double charge, int id)
      : _pos(pos), _charge(charge), _id(id) {}
...
public:
  ...
  Graph(std::vector<Particle> particles,
        std::vector<ParticleData> particle_data, std::vector<Edge> edges) {
  ...
  Graph(std::string file_type, std::string file_path) {
    ...
    if (file_type == "standard") {
      // reads file. first line is int with number of nodes and then each line
      // has two ints the endpoints of an edge
      ...
      for (int i = 0; i < id_number; i++) {
        _particles.emplace_back(
            Particle(std::complex<double>((double)rand() / RAND_MAX,
      ...
      std::cout << " Graph init recieved bad file format, valid formats are: "
                   "standard, gml";
    }
  }
  ...
  int num_vertices() { return _particles.size(); }
  ...
  std::vector<std::vector<double>> floyd_warschall() {
    ...
    std::vector<std::vector<double>> distances(
        _particles.size(),
        std::vector<double>(_particles.size(),
                            std::numeric_limits<double>::max()));

    // init distances matrix with zeros for nodes to itself and edge length from
    // nodes to neighbours
    ...
    for (int k = 0; k < _particles.size(); k++) {
      for (int i = 0; i < _particles.size(); i++) {
        for (int j = 0; j < _particles.size(); j++) {
          if (distances[i][k] > 0 &&
              distances[k][j] >
                  std::numeric_limits<double>::max() - distances[i][k]) {
            distances[i][j] =
                std::min(distances[i][j], std::numeric_limits<double>::max());
          } else {
            distances[i][j] =
  ...
  get_components() {
    ...
    class get {
    public:
      static void get_neighbours(int node, int val,
    ...
    int num_components =
        std::set<double>(parent_vec.begin(), parent_vec.end()).size();
    ...
    for (int i = 0; i < parent_vec.size(); i++) {
      ...
      _components[parent_vec[i]].add_particle(
          Particle(_particles[i].get_pos(), _particles[i].get_charge(), new_id),
          _particle_data[i]);
      _new_id[i] = new_id;
    }
    for (auto e : _edges) {
    ...
    // components.begin(), value_selector);
    ...
    return std::tuple<std::vector<Graph>, std::unordered_map<int, int>,
                      std::vector<int>>(components, _new_id, component_nr);
  }
  ...
  Graph timestamp_subgraph(int timestamp) {
    ...
    for (int i = 0; i < _particles.size(); i++) {
      if (part_mask[i]) {
        ...
        subgraph.add_particle(Particle(_particles[i].get_pos(),
                                       _particles[i].get_charge(), new_id),
                              _particle_data[i]);
        _new_id[i] = new_id;
      }
    }
